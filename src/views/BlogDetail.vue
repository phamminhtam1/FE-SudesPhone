...
const {blogpost, categoryBlogPost} = storeToRefs(blogPostStore)
...
watch(blogpost, (newBlogpost) => {
  if (newBlogpost && newBlogpost.meta_title) {
    emit('update-product-name', {
      ...
      categoryBlogName: newBlogpost.category_blog?.name,
  ...
  <div class=" flex max-w-[1260px] mx-auto mt-5 gap-7">
    <div class="w-3/4">
      ...
      <div class="mt-4 grid grid-cols-3 gap-5">
        <div class="col-span-1 border border-zinc-200 rounded-lg overflow-hidden relative shadow-xl">
          ...
          <div class="px-4 py-2">
            <span class="text-[15px] font-medium text-zinc-600 line-clamp-2 capitalize">iPhone 15 Ultra sẽ 'cắt đuôi' đối thủ nhờ trang bị 'có một không hai' này</span>
            <span class="text-[13px] font-medium mt-2 text-zinc-400 line-clamp-3">Cải thiện khả năng chụp ảnh luôn là ưu tiên hàng đầu của Apple trong mỗi dịp ra mắt iPhone. Năm nay, mẫu iPhone cao cấp nhất có thể sẽ sở hữu một trang bị cực kỳ đáng tiền.</span>
          </div>
        </div>
        <div class="col-span-1 border border-zinc-200 rounded-lg overflow-hidden relative shadow-xl">
          <img src="https://bizweb.dktcdn.net/100/480/632/articles/003.jpg?v=1682691879797" alt=""
    ...
    <div class="w-1/4 pr-3">
      <div class="border border-zinc-200 rounded-lg flex flex-col shadow-lg">
        ...
        <span class="px-3 py-2 font-medium text-zinc-600 text-[15px] hover:text-red-700 cursor-pointer" v-for="categoryblog in categoryBlogPost" :key="categoryblog.id">{{ categoryblog.name }}</span>
      </div>
      <div class="border border-zinc-200 rounded-lg flex flex-col shadow-lg mt-5">
        ...
        <div class="flex items-center justify-between w-full gap-2 p-2">
          <img src="https://bizweb.dktcdn.net/100/480/632/articles/010.jpg?v=1682692969433" alt=""
            class="w-[98px] h-[56px] rounded-lg">
          <div class="flex flex-col justify-between h-full">
            <span class=" cursor-pointer hover:text-red-700 text-[12px] font-medium text-zinc-600 line-clamp-2">Apple
              'in tiền' đỉnh
              thế nào: Cá kiếm từ cả
              điện thoại cũ lẫn mới,...</span>
            <span class="text-[12px] text-zinc-500 italic">28/04/2023</span>
          </div>
        </div>
        <div class="flex items-center justify-between w-full gap-2 p-2">
          <img src="https://bizweb.dktcdn.net/100/480/632/articles/010.jpg?v=1682692969433" alt=""
