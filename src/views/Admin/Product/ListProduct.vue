<script setup>
import LeftMenu from '@/components/layout/backend/LeftMenu.vue';
import HeaderAdmin from '@/components/layout/backend/HeaderAdmin.vue';
import ShowProductDetail from './ShowProductDetail.vue';
import Swal from 'sweetalert2';
import { ref, onMounted } from 'vue';
import axios from '@/plugins/axion';
import router from '@/router';
import { watch } from 'vue';

const selectedProductId = ref(null)
const showModal = ref(false)

function showProductDetail(id) {
  const url = router.resolve({ name: 'show_product_detail', params: { id } }).href
  window.open(url, '_blank')
}

function goEditProduct(id) {
  router.push({ name: 'edit_product', params: { id } })
}

function resetFilters() {
  keyword.value = ''
  selectedCategory.value = ''
  selectedColor.value = ''
  selectedPriceType.value = ''
  minPrice.value = ''
  maxPrice.value = ''
}

function showQrPopup(qrUrl) {
  Swal.fire({
    title: 'Mã QR Sản Phẩm',
    imageUrl: qrUrl,
    imageAlt: 'QR Code',
    imageWidth: 300,
    imageHeight: 300,
    confirmButtonText: 'Đóng',
    showCloseButton: true,
    background: '#fff',
    backdrop: true
  });
}

function getPageNumbers() {
  const pages = []
  const maxVisiblePages = 5
  if (lastPage.value <= maxVisiblePages) {
    for (let i = 1; i <= lastPage.value; i++) {
      pages.push(i)
    }
  } else {
    pages.push(1)
    if (currentPage.value > 3) {
      pages.push('...')
    }
    const start = Math.max(2, currentPage.value - 1)
    const end = Math.min(lastPage.value - 1, currentPage.value + 1)
    for (let i = start; i <= end; i++) {
      pages.push(i)
    }
    if (currentPage.value < lastPage.value - 2) {
      pages.push('...')
    }
    if (lastPage.value > 1) {
      pages.push(lastPage.value)
    }
  }
  return pages
}

const products = ref([])
const error = ref(false)
const cat_id = ref('')
const selectedCategory = ref('')
const selectedColor = ref('')
const keyword = ref('')
const selectedPriceType = ref('')
const selectedPerPage = ref(10)
const minPrice = ref('')
const maxPrice = ref('')

const currentPage = ref(1)
const lastPage = ref(1)
const total = ref(0)
const from = ref(0)
const to = ref(0)

watch([keyword, selectedCategory, selectedColor, selectedPriceType, minPrice, maxPrice, selectedPerPage], async () => {
  currentPage.value = 1
  await featchProducts()
})

async function featchProducts() {
  error.value = null
  try {
    const params = {
      page: currentPage.value,
      per_page: selectedPerPage.value
    }
    if (keyword.value) {
      params.keyword = keyword.value
    }
    if (selectedCategory.value) {
      params.cat_id = selectedCategory.value
    }
    if (selectedColor.value) {
      params.spec_key = 'Màu sắc'
      params.spec_value = selectedColor.value
    }
    if (selectedPriceType.value) {
      params.price_type = selectedPriceType.value
    }
    if (minPrice.value) {
      params.min_price = minPrice.value
    }
    if (maxPrice.value) {
      params.max_price = maxPrice.value
    }
    const res = await axios.get('/api/product', { params })
    products.value = res.data.product.data
    currentPage.value = res.data.product.current_page
    lastPage.value = res.data.product.last_page
    total.value = res.data.product.total
    from.value = res.data.product.from
    to.value = res.data.product.to
  } catch (err) {
    error.value = err.response?.data?.message
  }
}

async function goToPage(page) {
  if (page >= 1 && page <= lastPage.value) {
    currentPage.value = page
    await featchProducts()
  }
}

async function featchCategory() {
  try {
    const res = await axios.get('/api/category/leaf-only')
    cat_id.value = res.data.category
  } catch (err) {
    console.error(err)
  }
}

async function updateProductStatus(id, newStatus) {
  try {
    await axios.put(`/api/product/update-status/${id}`, {
      status: newStatus
    })
    await featchProducts()
  } catch (err) {
    console.error(err)
    error.value = err.response?.data?.message || 'Có lỗi xảy ra khi cập nhật trạng thái'
  }
}

async function deleteProduct(id) {
  const result = await Swal.fire({
    title: 'Xác nhận xóa',
    text: 'Bạn có chắc chắn muốn xóa sản phẩm này?',
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#d33',
    cancelButtonColor: '#3085d6',
    confirmButtonText: 'Xóa',
    cancelButtonText: 'Hủy'
  })
  if (result.isConfirmed) {
    try {
      await axios.delete(`api/product/delete/${id}`)
      await featchProducts()
      Swal.fire('Đã xóa!', 'Sản phẩm này đã được xóa thành công.', 'success')
    } catch (err) {
      error.value = err.response?.data?.message || 'Có lỗi xảy ra khi xóa vị trí'
    }
  }
}
onMounted(featchProducts)
onMounted(featchCategory)

</script>
<template>
  <LeftMenu />
  <HeaderAdmin />
  <div class="ml-57 min-h-screen bg-[#F3F3F3] pt-5">
    <div class="mx-5 border bg-white border-zinc-300 rounded-lg">
      <div class="flex justify-between p-2">
        <div class="flex p-3">
          <svg fill="#2563EB" viewBox="0 0 32 32" width="17px" height="17px" class="mt-1 mr-2"
            xmlns="http://www.w3.org/2000/svg">
            <path
              d="M12,25l6.67,6.67a1,1,0,0,0,.7.29.91.91,0,0,0,.39-.08,1,1,0,0,0,.61-.92V13.08L31.71,1.71A1,1,0,0,0,31.92.62,1,1,0,0,0,31,0H1A1,1,0,0,0,.08.62,1,1,0,0,0,.29,1.71L11.67,13.08V24.33A1,1,0,0,0,12,25ZM3.41,2H28.59l-10,10a1,1,0,0,0-.3.71V28.59l-4.66-4.67V12.67a1,1,0,0,0-.3-.71Z">
            </path>
          </svg>
          <span class="font-medium">Tìm kiếm và lọc sản phẩm</span>
        </div>
        <div class="flex p-3 text-sm items-center cursor-pointer hover:text-red-500 transition duration-200"
          @click="resetFilters">
          <span>Xóa bộ lọc</span>
        </div>

      </div>
      <div class="flex justify-between w-full mb-2 p-2">
        <div class="p-3 w-1/3">
          <span class="text-[12px] font-medium block mb-2">Tìm kiếm</span>
          <div class="flex border border-zinc-300 rounded-lg h-10 justify-start items-center">
            <svg viewBox="0 0 32 32" version="1.1" width="13px" height="13px" class="mx-3 mt-0.5"
              xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
              xmlns:sketch="http://www.bohemiancoding.com/sketch/ns" fill="#6b6b6b" stroke="#6b6b6b">
              <g id="Icon-Set" sketch:type="MSLayerGroup" transform="translate(-256.000000, -1139.000000)"
                fill="#6b6b6b">
                <path
                  d="M269.46,1163.45 C263.17,1163.45 258.071,1158.44 258.071,1152.25 C258.071,1146.06 263.17,1141.04 269.46,1141.04 C275.75,1141.04 280.85,1146.06 280.85,1152.25 C280.85,1158.44 275.75,1163.45 269.46,1163.45 L269.46,1163.45 Z M287.688,1169.25 L279.429,1161.12 C281.591,1158.77 282.92,1155.67 282.92,1152.25 C282.92,1144.93 276.894,1139 269.46,1139 C262.026,1139 256,1144.93 256,1152.25 C256,1159.56 262.026,1165.49 269.46,1165.49 C272.672,1165.49 275.618,1164.38 277.932,1162.53 L286.224,1170.69 C286.629,1171.09 287.284,1171.09 287.688,1170.69 C288.093,1170.3 288.093,1169.65 287.688,1169.25 L287.688,1169.25 Z"
                  id="search" sketch:type="MSShapeGroup"> </path>
              </g>
            </svg>
            <input v-model="keyword" type="text" class="w-full focus:outline-none focus:ring-0"
              placeholder="Tìm kiếm theo tên, mã kho...">
          </div>
        </div>
        <div class="p-3 w-2/3 flex justify-between overflow-hidden">
          <div class="w-1/4 mr-3">
            <span class="text-[12px] font-medium block mb-2">Danh mục</span>
            <div>
              <select v-model="selectedCategory"
                class="border text-zinc-700 border-zinc-300 w-full h-10 rounded-lg pl-3 pr-8 mr-2">
                <option value="">Tất cả danh mục</option>
                <option v-for="cat in cat_id" :key="cat.id" :value="cat.id">{{ cat.name }}</option>
              </select>
            </div>
          </div>
          <div class="w-1/4 mr-3">
            <span class="text-[12px] font-medium block mb-2">Màu sắc</span>
            <div>
              <select v-model="selectedColor"
                class="border text-zinc-700 border-zinc-300 w-full h-10 rounded-lg pl-3 pr-8">
                <option value="">Tất cả màu sắc</option>
                <option value="Đen">Đen</option>
                <option value="Trắng">Trắng</option>
                <option value="Vàng">Vàng</option>
                <option value="Đỏ">Đỏ</option>
                <option value="Xanh">Xanh</option>
                <option value="Tím">Tím</option>
                <option value="Hồng">Hồng</option>
              </select>
            </div>
          </div>
          <div class="w-1/4 mr-3">
            <span class="text-[12px] font-medium block mb-2">Loại giá</span>
            <div>
              <select v-model="selectedPriceType"
                class="border text-zinc-700 border-zinc-300 w-full h-10 rounded-lg pl-3 pr-10">
                <option value="">Tất cả loại giá</option>
                <option value="price">Giá gốc</option>
                <option value="discount_price">Giá bán</option>
              </select>
            </div>
          </div>
          <div class="w-1/4 mr-3">
            <span class="text-[12px] font-medium block mb-2 ">Khoảng giá (VND)</span>
            <div class="flex justify-start w-full">
              <input type="text" class="border w-[calc(50%-0.25rem)] border-zinc-300 h-10 rounded-lg mr-2 pl-2"
                placeholder="Từ" v-model="minPrice">
              <input type="text" class="border w-[calc(50%-0.25rem)] border-zinc-300 h-10 rounded-lg pl-2"
                placeholder="Đến" v-model="maxPrice">
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="mx-5 mt-5 border border-zinc-300 bg-white rounded-lg">
      <div class="flex justify-between mb-1 p-4">
        <div class=" flex justify-between items-center">
          <select v-model="selectedPerPage" class="border text-zinc-700 border-zinc-400 w-12 h-7 rounded-lg">
            <option value="10">10</option>
            <option value="20">20</option>
            <option value="50">50</option>
            <option value="100">100</option>
          </select>
        </div>
        <div class="flex">
          <div class="mr-3 mt-1 text-zinc-600">
            Tổng cộng: <span class="text-[#2563EB]">{{ total }}</span> sản phẩm
          </div>
          <router-link to="/admin/category/product/create">
            <button class="border rounded-lg px-4 flex py-1.5 bg-[#2563EB] text-zinc-100 cursor-pointer
               hover:-translate-y-0.5 transition duration-300 ">
              <svg viewBox="0 0 24 24" fill="none" width="24px" height="24px" class="mr-2"
                xmlns="http://www.w3.org/2000/svg" stroke="#f5f5f5">
                <path fill-rule="evenodd" clip-rule="evenodd"
                  d="M11.25 12.75V18H12.75V12.75H18V11.25H12.75V6H11.25V11.25H6V12.75H11.25Z" fill="#ededed"></path>
              </svg>
              <span class="text-[16px] font-medium">Thêm sản phẩm</span>
            </button>
          </router-link>
        </div>
      </div>
      <div class="overflow-x-auto ">
        <table class="table-auto w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
          <thead class="text-xs bg-[#384153] uppercase text-white dark:bg-gray-700 dark:text-gray-400">
            <tr class="">
              <th scope="col" class="px-2 py-3 w-12">
                <span class="flex items-center justify-center">STT</span>
              </th>
              <th scope="col" class="px-2 py-3 w-33 border-l">
                <span class="flex items-center justify-center">Tên sản phẩm</span>
              </th>
              <th scope="col" class="px-2 py-3 w-32 border-l">
                <span class="flex items-center justify-center">Ảnh thubnail</span>
              </th>
              <th scope="col" class="px-2 py-3 w-30 border-l">
                <span class="flex items-center justify-center">Danh mục</span>
              </th>
              <th scope="col" class="px-2 py-3 w-30 border-l">
                <span class="flex items-center justify-center">giá gốc</span>
              </th>
              <th scope="col" class="px-2 py-3 w-30 border-l">
                <span class="flex items-center justify-center">Giá bán</span>
              </th>
              <th scope="col" class="px-2 py-3 w-25 border-l">
                <span class="flex items-center justify-center">Sẵn kho</span>
              </th>
              <th scope="col" class="px-2 py-3 w-25 border-l">
                <span class="flex items-center justify-center">Xuất mã kho</span>
              </th>
              <th scope="col" class="px-2 py-3 w-25 border-l">
                <span class="flex items-center justify-center">Trạng thái</span>
              </th>
              <th scope="col" class="px-2 py-3 w-25 border-l">
                <span class="flex items-center justify-center">Hành động</span>
              </th>
            </tr>
          </thead>
          <tbody class="font-medium text-zinc-500 text-[13px]">
            <tr v-for="(product, index) in products" :key="product.prod_id"
              class="bg-white border-b text-sm hover:bg-zinc-200 transition dark:bg-gray-800 dark:border-gray-700 border-gray-200">
              <th scope="row" class="px-6 py-4 whitespace-nowrap dark:text-white">
                {{ index + 1 }}
              </th>
              <td class="px-6 py-4 text-zinc-700 font-bold">
                {{ product.name }}
              </td>
              <td class="px-6 py-4">
                <img v-if="product.images.length" :src="product.images[1].img_url" alt="product image"
                  class="w-full h-[110px] object-contain rounded-2xl" />
              </td>
              <td class="px-6 py-4 text-zinc-700 font-medium">
                <div class="flex justify-center items-center">
                  <span class="border-grenn-600 rounded-4xl bg-[#DBEAFE] w-20 h-15 flex justify-center items-center">
                    <p class=" text-[#3554B9] ml-2">{{ product.category?.name }}
                    </p>
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-red-700 font-medium">
                <div class="flex justify-center items-center">
                  <span class="border-grenn-600 rounded-2xl bg-red-600 w-full h-7 flex justify-center items-center">
                    <p class=" text-white">{{ new Intl.NumberFormat('vi-VN').format(product.price) }}đ
                    </p>
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-green-700 font-medium">
                <div class="flex justify-center items-center">
                  <span class="border-grenn-600 rounded-2xl bg-green-600 w-full h-7 flex justify-center items-center">
                    <p class=" text-white">{{ new Intl.NumberFormat('vi-VN').format(product.discount_price) }}đ
                    </p>
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-zinc-700 font-medium">
                <div class="flex justify-center items-center">
                  <span class="border-grenn-600 rounded-2xl bg-[#DCFCE7] w-9 h-6 flex justify-center items-center">
                    <p class=" text-[#247041]">{{ product.stock_qty }}</p>
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-zinc-700 font-medium">
                <div class="flex justify-center items-center">
                  <span @click="showQrPopup(product.qr_path)"
                    class="border-grenn-600 rounded-2xl bg-[#DCFCE7] w-9 h-6 flex justify-center items-center">
                    <img class="cursor-pointer" :src="product.qr_path" alt="">
                  </span>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex justify-center items-center">
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" :checked="product.status === 1"
                      @change="updateProductStatus(product.prod_id, product.status === 1 ? 0 : 1)" class="sr-only peer">
                    <div
                      class="w-11 h-6 bg-gray-200 peer-focus:outline-none  rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white  after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#384153]">
                    </div>
                  </label>
                </div>
              </td>
              <td class="px-6 py-4 max-w-30">
                <div class="flex justify-center items-center gap-x-1">
                  <button @click="showProductDetail(product.prod_id)">
                    <span
                      class="w-9 pb-1 mt-1 flex items-center justify-center h-8 rounded-lg cursor-pointer hover:-translate-y-0.5 transition duration-300">
                      <svg viewBox="0 0 24 24" fill="none" width="26px" height="26px"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                          d="M12 5C6.55576 5 3.53109 9.23425 2.45554 11.1164C2.23488 11.5025 2.12456 11.6956 2.1367 11.9836C2.14885 12.2716 2.27857 12.4598 2.53799 12.8362C3.8182 14.6935 7.29389 19 12 19C16.7061 19 20.1818 14.6935 21.462 12.8362C21.7214 12.4598 21.8511 12.2716 21.8633 11.9836C21.8754 11.6956 21.7651 11.5025 21.5445 11.1164C20.4689 9.23425 17.4442 5 12 5Z"
                          stroke="#33363F" stroke-width="2"></path>
                        <circle cx="12" cy="12" r="4" fill="#33363F"></circle>
                      </svg>
                    </span>
                  </button>
                  <button @click="goEditProduct(product.prod_id)">
                    <span
                      class=" w-9 pb-1 flex items-center justify-center h-8 rounded-lg cursor-pointer hover:-translate-y-0.5 transition duration-300">
                      <svg viewBox="0 0 24 24" fill="none" width="26px" height="26px"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                          d="M21.1213 2.70705C19.9497 1.53548 18.0503 1.53547 16.8787 2.70705L15.1989 4.38685L7.29289 12.2928C7.16473 12.421 7.07382 12.5816 7.02986 12.7574L6.02986 16.7574C5.94466 17.0982 6.04451 17.4587 6.29289 17.707C6.54127 17.9554 6.90176 18.0553 7.24254 17.9701L11.2425 16.9701C11.4184 16.9261 11.5789 16.8352 11.7071 16.707L19.5556 8.85857L21.2929 7.12126C22.4645 5.94969 22.4645 4.05019 21.2929 2.87862L21.1213 2.70705ZM18.2929 4.12126C18.6834 3.73074 19.3166 3.73074 19.7071 4.12126L19.8787 4.29283C20.2692 4.68336 20.2692 5.31653 19.8787 5.70705L18.8622 6.72357L17.3068 5.10738L18.2929 4.12126ZM15.8923 6.52185L17.4477 8.13804L10.4888 15.097L8.37437 15.6256L8.90296 13.5112L15.8923 6.52185ZM4 7.99994C4 7.44766 4.44772 6.99994 5 6.99994H10C10.5523 6.99994 11 6.55223 11 5.99994C11 5.44766 10.5523 4.99994 10 4.99994H5C3.34315 4.99994 2 6.34309 2 7.99994V18.9999C2 20.6568 3.34315 21.9999 5 21.9999H16C17.6569 21.9999 19 20.6568 19 18.9999V13.9999C19 13.4477 18.5523 12.9999 18 12.9999C17.4477 12.9999 17 13.4477 17 13.9999V18.9999C17 19.5522 16.5523 19.9999 16 19.9999H5C4.44772 19.9999 4 19.5522 4 18.9999V7.99994Z"
                          fill="#3d3d3d"></path>
                      </svg>
                    </span>
                  </button>
                  <button @click="deleteProduct(product.prod_id)">
                    <span
                      class="flex items-center justify-center w-8 h-7 -mt-1 rounded-lg cursor-pointer hover:-translate-y-0.5 transition duration-300">
                      <svg width="26px" height="26px" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="22" height="22" rx="5" fill="url(#pattern0_48_276)" />
                        <defs>
                          <pattern id="pattern0_48_276" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_48_276" transform="scale(0.0078125)" />
                          </pattern>
                          <image id="image0_48_276" width="128" height="128" preserveAspectRatio="none"
                            xlink:href="data:image/png;base64," />
                        </defs>
                      </svg>
                    </span>
                  </button>
                </div>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <ShowProductDetail v-if="showModal" :id="selectedProductId" @close="showModal = false" />
    </div>
    <div v-if="lastPage > 1" class="flex items-center justify-between px-6 py-4 ">
      <div class="text-sm text-gray-700">
        Hiển thị {{ from }} đến {{ to }} trong tổng số <span class="text-[#2563EB]"> {{ total }}</span> kết quả
      </div>
      <div class="flex items-center space-x-2">
        <!-- Previous button -->
        <button @click="goToPage(currentPage - 1)" :disabled="currentPage === 1"
          class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
          Trước
        </button>

        <!-- Page numbers -->
        <template v-for="page in getPageNumbers()" :key="page">
          <button v-if="page !== '...'" @click="goToPage(page)" :class="[
            'px-3 py-2 text-sm font-medium rounded-md',
            currentPage === page
              ? 'bg-[#2563EB] text-white'
              : 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-50'
          ]">
            {{ page }}
          </button>
          <span v-else class="px-3 py-2 text-sm text-gray-500">...</span>
        </template>

        <!-- Next button -->
        <button @click="goToPage(currentPage + 1)" :disabled="currentPage === lastPage"
          class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
          Sau
        </button>
      </div>
    </div>
  </div>
</template>

<style>
/* Chỉ mảnh 4px, bo tròn nhẹ, màu trung tính */


/* Firefox */
.email-scroll {
  scrollbar-width: thin;
  scrollbar-color: #9ca3af transparent;
}
</style>
