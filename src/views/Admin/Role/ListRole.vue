...
async function fetchRoles() {
  try {
    ...
    const res = await axios.get('/api/role', { params })
...
async function updateStatusRole(id, newStatus) {
  try {
    await axios.put(`/api/role/update-status/${id}`, {
      status: newStatus
    })
    await fetchRoles()
  } catch (err) {
...
function goEditRole(id) {
  router.push({ name: 'edit_role', params: { id } })
}
...
<template>
  <LeftMenu />
  <HeaderAdmin />
  <div class="ml-57 min-h-screen pt-5 bg-[#F3F3F3] flex justify-start">
    <div class="flex-1 overflow-x-auto mx-5 border border-zinc-300 bg-white rounded-lg">
      <div class="flex justify-between mb-2 mt-1 p-4">
        <div class="flex border w-1/3 border-zinc-300 rounded-lg h-10 justify-start items-center">
        ...
        <div class="flex">
          <div class="mr-3 mt-1 text-zinc-600">
            Tổng cộng: <span class="text-[#2563EB]">{{ roles.length }}</span> chức vụ
          </div>
          <router-link to="/admin/user/role/create">
            <button class="border rounded-lg px-4 flex py-1.5 bg-[#2563EB] text-zinc-100 cursor-pointer
              ...
              <span class="text-[16px] font-medium">Thêm chức vụ</span>
            </button>
          </router-link>
        </div>
      </div>
      <table class="table-fixed w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs uppercase bg-[#384153] text-white dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" class="px-6 py-3 w-35 border border-l-2px">
              Tên chức vụ
            </th>
            <th scope="col" class="px-6 py-3 w-100 border border-l-2px">
              Mô tả
            </th>
          ...
          <tr v-for="(role, index) in roles" :key="role.id"
            ...
            <td class="px-6 py-4 ">
              <label class="relative inline-flex items-center ml-3 cursor-pointer">
                <input type="checkbox" :checked="role.status === 1"
                  @change="updateStatusRole(role.id, role.status === 1 ? 0 : 1)" class="sr-only peer">
                <div
            ...
            <td class="px-6 py-4 flex justify-center">
              <div class="flex">
                <button @click="goEditRole(role.id)">
                  <span
