...
function goEditSupplier(id) {
  router.push({ name: 'edit_supplier', params: { id } })
}
...
<template>
  ...
  <div class="ml-57 min-h-screen bg-[#F3F3F3] flex justify-start pt-5">
    <div class="flex-1 min-h-screen border rounded-lg border-zinc-300 bg-white p-4 mx-6 overflow-x-auto">
      <div class="flex justify-between mb-5 mt-1">
        <div class="w-1/3">
          <div class="flex border border-zinc-400 rounded-lg h-10 justify-start items-center">
        ...
        <div class="flex">
          <div class="mr-3 mt-1 text-zinc-600">
            Tổng cộng: <span class="text-[#2563EB]">{{ suppliers.length }}</span> nhà cung cấp
          </div>
          <router-link to="/admin/purchase/supplier/create">
            <button class="border rounded-lg px-4 flex py-1.5 bg-[#2563EB] text-zinc-100 cursor-pointer
              ...
              <span class="text-[16px] font-medium">Thêm nhà cung cấp</span>
            </button>
          </router-link>
        </div>
      </div>
      <div>
        <table class="table-fixed w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody class="font-medium text-zinc-500 text-[14px]">
            <tr v-for="(supplier, index) in suppliers" :key="supplier.supp_id"
              ...
              <td class="px-6 py-4 ">
                <label class="relative inline-flex items-center ml-3 cursor-pointer">
                  <input type="checkbox" :checked="supplier.status === 1"
                    @change="updateStatusSupplier(supplier.supp_id, supplier.status === 1 ? 0 : 1)"
                    class="sr-only peer">
                  <div
              ...
              <td class="px-6 py-4">
                <div class="flex">
                  <button @click="goEditSupplier(supplier.supp_id)">
                    <span
